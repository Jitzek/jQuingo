...
export class jQuingo {
    public static createNodes(
        ...
        const element =
            _html && $(_html)[0] ? $(_html)[0].parentNode : _element;
        if (!element) return [];
        ...
        for (let i = 0; i < element.children.length; i++) {
            ...
            node.children = node.children.concat(
                this.createNodes(undefined, child)
            );
            ...
            if (node instanceof jQuingoComponentNode) {
                if (new_node instanceof jQuingoComponentNode) {
                    ...
                    if (node.props !== new_node.props)
                        (nodes[i] as jQuingoComponentNode).props =
                            new_node.props;
                    (nodes[i] as jQuingoComponentNode).children =
                        this.updateNodes(node.children, new_node.children);
